...
| Phase 18: Credential Vault | ✅ COMPLETE | 45/45 ✓ | nexus/credentials/: encryption.py (Fernet AES-256, ephemeral key + warning), vault.py (CredentialVault: CRUD, inject_credentials by type, get_env_vars for MCP subprocess injection, refresh_oauth2), sanitize_tool_params() strips secrets before ledger seal; ToolExecutor wired to inject credentials post-gates pre-sandbox |
| Phase 19: MCP Integration | ✅ COMPLETE | 118/118 ✓ | nexus/mcp/: client.py (MCPClient: stdio/sse/streamable_http, namespaced tool names mcp_{server}_{tool}, AsyncExitStack session lifecycle, env merge not replace), adapter.py (MCPToolAdapter: register_server, unregister_server, reconnect_all, vault credential injection via _inject_credential + get_env_vars); registry.py source tracking + unregister + get_by_source; MCPServerConfig.credential_id field; fixtures: mcp_echo_server, mcp_fetch_server, mcp_env_server; examples: mcp_integration/main.py + context7_demo.py; proven live: sequential-thinking + Context7 real subprocesses |
| Phase 20: Universal HTTP Tool + Data Transform | ✅ COMPLETE | 74/74 ✓ | nexus/tools/builtin/http_request.py (method/url/headers/body_format/pagination/retry/JMESPath/auth) + data_transform.py (15 ops: filter/map/sort/group_by/flatten/pick/omit/rename/deduplicate/limit/skip/aggregate/merge/set/cast); jmespath dep added |
| Phase 21: Code Sandbox v2 | ✅ COMPLETE | 67/67 ✓ | nexus/tools/sandbox_v2.py: execute_python (ast import validation, venv, RLIMIT_AS) + execute_javascript (ESM/CJS, network isolation); registered in _registered_tools |
| Phase 22: Trigger System | ✅ COMPLETE | — | nexus/triggers/: webhook, cron scheduler, event bus, workflow-complete chaining |
| Phase 23: NL Workflow Generation | ✅ COMPLETE | 54/54 ✓ | nexus/workflows/generator.py: generate/refine/explain, 4-strategy JSON extraction, _pre_validate(); 3 new prompts |
| Phase 23.1: Ambiguity Resolution | ✅ COMPLETE | 86/86 ✓ | nexus/workflows/ambiguity.py: AmbiguityResolver (score/start/submit/get/cancel/generate); AmbiguitySession, SpecificityScore, WorkflowPlan types; sealed plan SHA256 fingerprint |
| Phase 24: Visual Canvas | ✅ COMPLETE | 22/22 frontend ✓ | ReactFlow drag-and-drop workflow editor; WorkflowEditor.jsx (32 frontend files); /v2/workflows CRUD routes; vite.config.js /v2 proxy; nodeTypes/edgeTypes at module level |
| Phase 25: Frontend v2 + Skills System | ✅ COMPLETE | 79/79 backend + 35/35 frontend ✓ | nexus/skills/manager.py (SkillManager: CRUD, auto version bump, SKILL.md import, semantic match); 5 new /v2 route files (skills, credentials, mcp_servers, executions, events SSE); WorkflowGenerator skill context injection; frontend: Zustand store, React Query hooks, Sidebar/TopBar/MobileNav layout, GateChip+GateBar 4-gate visualization, ChainReplay, Skills/Credentials/MCPServers/Executions pages, Workflows page with Generate+Import+Templates toolbar; ruff clean |

**Last verified:** 2026-02-24
**Test suite:** `.venv312/bin/pytest tests/ -m "not slow"` → **1578/1578 passed**
