...
| Phase 22: Trigger System | ✅ COMPLETE | 63/63 ✓ | nexus/triggers/: EventBus (subscribe/emit, workflow.completed · workflow.failed · seal.blocked), TriggerManager (create/enable/disable/delete/list + fire → engine.run_workflow), CronScheduler (croniter, async start/stop, check_and_fire), WebhookHandler (path lookup → fire); TriggerConfig CRUD in repository; workflow-complete chaining; wired in lifespan |
| Phase 23: NL Workflow Generation | ✅ COMPLETE | 54/54 ✓ | nexus/workflows/generator.py — WorkflowGenerator: generate() (up to N retry attempts, passes previous_errors on failure), refine() (single-shot), explain() (plain-English); _parse_json() 4-strategy extraction; _pre_validate() structural checks; max_generation_refine_attempts=3; WorkflowGenerationError; prompts: GENERATE_WORKFLOW, REFINE_WORKFLOW, EXPLAIN_WORKFLOW |
| Phase 23.1: Ambiguity Resolution | ✅ COMPLETE | 87/87 ✓ | nexus/workflows/ambiguity.py — AmbiguityResolver: score() (LLM + heuristics, no DB), start_session(), submit_answers() (typed validation: single_choice/multi_choice/boolean/number/text), get_session/cancel_session, plan_to_generator_context(), _seal_plan() (SHA256); multi-round clarification (max_clarification_rounds=3); AmbiguitySession DB model + 4 repo methods; API routes: score, start, get, list, submit-answers, cancel, generate |
| Phase 24: Visual Canvas | ✅ COMPLETE | 22/22 ✓ (frontend) | frontend/src/pages/WorkflowEditor.jsx — ReactFlow v11 canvas, 8 node types, 3 edge types, drag-and-drop, dagre auto-layout, PropertiesPanel, NodeToolbox, NLGenerateModal, ValidationModal, VersionHistoryPanel, GateStatusOverlay; backend 6 CRUD routes (/v2/workflows), CORS PATCH fix, vite /v2 proxy, api.js v2 path detection; Workflows list page |
...
**Test suite:** `.venv312/bin/pytest tests/ -m "not slow"` → **1499/1499 passed** (v1 phases 0-14 + v2 phases 15-23.1 + coverage gap suite)
